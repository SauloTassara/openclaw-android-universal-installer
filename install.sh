...
if ! command -v termux-wake-lock >/dev/null 2>&1; then
  warn "termux-wake-lock command not found. Install Termux:API app from F-Droid for reliable wake locks."
fi
...
if command -v openclaw >/dev/null 2>&1; then
  ...
  curl -fsSL https://myopenclawhub.com/install | bash || warn "OpenClaw installer failed. Check $LOG_FILE."
fi
...
if command -v openclaw >/dev/null 2>&1; then
  if ask_default_yes "Try OpenClaw beta update now?"; then
    ...
    openclaw update --channel beta --yes || warn "Beta update failed; stable/latest remains usable"
  fi
else
  warn "openclaw command not found after install. Skipping beta update."
fi
