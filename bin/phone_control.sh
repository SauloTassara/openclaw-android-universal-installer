...
run_android() {
  ...
  if command -v rish >/dev/null 2>&1 && RISH_APPLICATION_ID=com.termux rish -c 'id' >/dev/null 2>&1; then
    ...
    sh -c "$cmd" 2>/dev/null || die "No rish/adb available. Start Shizuku/Sui or connect adb wireless debugging."
  fi
}
...
case "$cmd" in
  home) run_android "/system/bin/input keyevent 3" ;;
  back) run_android "/system/bin/input keyevent 4" ;;
  recent) run_android "/system/bin/input keyevent 187" ;;
  tap)
    [ $# -eq 2 ] || die "tap needs X Y"
    is_uint "$1" && is_uint "$2" || die "tap coordinates must be non-negative integers"
    run_android "/system/bin/input tap $1 $2"
    ;;
  swipe)
    ...
    run_android "/system/bin/input swipe $1 $2 $3 $4 ${5:-300}"
    ;;
  text)
    [ $# -ge 1 ] || die "text needs text"
    encoded="$(printf "%s" "$*" | sed 's/%/%25/g; s/ /%s/g')"
    run_android "/system/bin/input text '$(quote_shell "$encoded")'"
    ;;
  open-app)
    [ $# -eq 1 ] || die "open-app needs package"
    valid_package "$1" || die "invalid Android package name"
    run_android "/system/bin/monkey -p '$(quote_shell "$1")' -c android.intent.category.LAUNCHER 1"
    ;;
  open-url)
    [ $# -eq 1 ] || die "open-url needs URL"
    valid_url "$1" || die "URL scheme must be http://, https://, geo:, market:, or intent:"
    run_android "/system/bin/am start -a android.intent.action.VIEW -d '$(quote_shell "$1")'"
    ;;
  screenshot)
    ...
    run_android "/system/bin/screencap -p '$(quote_shell "$path")'"
    echo "$path"
    ;;
  ui-dump)
    remote="/sdcard/openclaw_window_dump.xml"
    run_android "/system/bin/uiautomator dump '$remote' >/dev/null"
    run_android "/system/bin/cat '$remote'"
    ;;
  battery) run_android "/system/bin/dumpsys battery" ;;
  brightness)
    [ $# -eq 1 ] || die "brightness needs 0..255"
    is_uint "$1" || die "brightness must be numeric"
    [ "$1" -ge 0 ] && [ "$1" -le 255 ] || die "brightness must be 0..255"
    run_android "/system/bin/settings put system screen_brightness $1"
    ;;
  -h|--help|help) usage ;;
  *) usage; exit 1 ;;
esac
